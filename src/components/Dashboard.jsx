...
export const Dashboard = ({ data, schema, stats }) => {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* 4 Premium SaaS KPI Cards (Responsive grid) */}
      ...
      <div className="dashboard-grid">
        ...
        <div className="glass-card" style={{ gridColumn: 'span 2' }}>
          <div className="chart-header">
            ...
            <div className="chart-controls" style={{ gap: '0.35rem' }}>
              <select className="select-control" value={chartType} onChange={(e) => setChartType(e.target.value)} style={{ padding: '0.2rem 0.5rem', fontSize: '0.75rem', height: '30px', width: 'auto' }}>
                <option value="bar">Bar Chart</option>
                <option value="line">Line Chart</option>
                <option value="area">Area Chart</option>
              </select>

              <select className="select-control" value={chartXAxis} onChange={(e) => setChartXAxis(e.target.value)} style={{ padding: '0.2rem 0.5rem', fontSize: '0.75rem', height: '30px', width: 'auto' }}>
                {stats.categoricalColumns.map(c => <option key={c} value={c}>X-Axis: {c}</option>)}
                {stats.dateColumns.map(c => <option key={c} value={c}>X-Axis: {c}</option>)}
              </select>

              {stats.numericColumns.length > 0 && (
                <select className="select-control" value={chartYAxis} onChange={(e) => setChartYAxis(e.target.value)} style={{ padding: '0.2rem 0.5rem', fontSize: '0.75rem', height: '30px', width: 'auto' }}>
                  {stats.numericColumns.map(c => <option key={c} value={c}>Y-Axis: {c}</option>)}
                </select>
              )}

              <select className="select-control" value={chartAgg} onChange={(e) => setChartAgg(e.target.value)} style={{ padding: '0.2rem 0.5rem', fontSize: '0.75rem', height: '30px', width: 'auto' }}>
        ...
        {stats.categoricalColumns.length > 0 && (
          <div className="glass-card">
            <h3 className="chart-title" style={{ fontSize: '1.05rem', marginBottom: '0.75rem' }}>
              Composition Shares
            </h3>
          ...
          <div className="glass-card" style={{ gridColumn: 'span 2' }}>
            <div className="chart-header" style={{ marginBottom: '0.75rem' }}>
              ...
              <div className="chart-controls" style={{ gap: '0.25rem' }}>
                <select className="select-control" value={scatterX} onChange={(e) => setScatterX(e.target.value)} style={{ padding: '0.1rem 0.4rem', fontSize: '0.7rem', height: '26px', width: 'auto' }}>
                  {stats.numericColumns.map(c => <option key={c} value={c}>X: {c}</option>)}
                </select>
                <select className="select-control" value={scatterY} onChange={(e) => setScatterY(e.target.value)} style={{ padding: '0.1rem 0.4rem', fontSize: '0.7rem', height: '26px', width: 'auto' }}>
            ...
            <div style={{ width: '100%', height: 260 }}>
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 10, right: 10, bottom: 5, left: -20 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)" />
                  <XAxis type="number" dataKey="x" name={scatterX} stroke="var(--text-muted)" fontSize={9} />
                  <YAxis type="number" dataKey="y" name={scatterY} stroke="var(--text-muted)" fontSize={9} />
